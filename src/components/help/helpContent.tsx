...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="font-mono text-[10px] text-accent-light bg-obsidian-high px-1.5 py-0.5 rounded-sm border border-outline-variant/20 inline-block">
      {children}
    </kbd>
  );
}
...
function H1({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="font-mono text-[13px] text-accent-light tracking-widest uppercase mb-4">
      {children}
    </h2>
  );
}
...
function H2({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="font-mono text-[11px] text-text-primary tracking-wider uppercase mt-6 mb-2">
      {children}
    </h3>
  );
}
...
function P({ children }: { children: React.ReactNode }) {
  return (
    <p className="font-mono text-[11px] text-text-muted leading-relaxed mb-2">
      {children}
    </p>
  );
}
...
function OL({ children }: { children: React.ReactNode }) {
  return (
    <ol className="font-mono text-[11px] text-text-muted leading-relaxed list-decimal list-inside space-y-1.5 mb-2 [&_kbd]:mx-0.5">
      {children}
    </ol>
  );
}
...
function UL({ children }: { children: React.ReactNode }) {
  return (
    <ul className="font-mono text-[11px] text-text-muted leading-relaxed list-disc list-inside space-y-1.5 mb-2 [&_kbd]:mx-0.5">
      {children}
    </ul>
  );
}
  ...
  return (
    <div data-help-section="getting-started">
      ...
      <P>
        ...
        <em className="text-accent-light not-italic"> SAVED</em> when the last
      ...
      <P>
        Click <em className="text-accent-light not-italic">EXPORT</em> while in
        ...
        <div key={group} className="mt-5">
          ...
          <div className="space-y-1.5">
            {SHORTCUTS.filter((s) => s.group === group).map((s, i) => (
              <div
                key={`${group}-${i}`}
                className="flex items-start gap-3 py-1 border-b border-outline-variant/10 last:border-0"
              >
                ...
                <div className="flex-1">
                  ...
                  {s.context && (
                    <div className="font-mono text-[9px] text-text-ghost mt-0.5">
                      {s.context}
                    </div>
                  )}
                </div>
  ...
  return (
    <div data-help-section="library">
      ...
      <P>
        ...
        <em className="text-accent-light not-italic">PROPERTIES</em> panel.
      </P>
  ...
  return (
    <div data-help-section="3d">
      ...
      <P>
        ...
        <em className="text-accent-light not-italic">WALK</em> toolbar button
        or press <Kbd>E</Kbd>.
      </P>
      <OL>
        ...
        <li>Press <Kbd>E</Kbd> again (or click <em className="text-accent-light not-italic">ORBIT</em>) to return to orbit view</li>
      </OL>
      <P>
      ...
      <UL>
        <li>Click <em className="text-accent-light not-italic">+</em> in the room tabs to add a room</li>
      ...
      <P>
        Click <em className="text-accent-light not-italic">EXPORT</em> from the
