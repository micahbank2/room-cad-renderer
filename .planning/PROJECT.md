...
**v1.20 Phase 78 (PBR Maps) shipped 2026-05-13.** Material system extended to support AO (ambient occlusion) and displacement maps end-to-end — 5-map PBR pipeline complete (color + roughness + reflection + AO + displacement). Wave 1: extended `Material` type + `SaveMaterialInput` with `aoMapId`/`displacementMapId`, threaded through `surfaceMaterial.ts` + `useResolvedMaterial.ts`; added AO_MAP + DISPLACEMENT_MAP drop zones to `UploadMaterialModal` with full object-URL lifecycle. Wave 2: bound `aoMap` + `displacementMap` + `displacementScale=0.05` + `uv2` BufferAttribute on `WallMesh`/`FloorMesh`/`CeilingMesh`/`CustomElementMesh` (ProductMesh/ProductBox excluded — use legacy `useProductTexture` path, future-phase migration); added `MapBadge` presence indicators (COLOR/ROUGH/REFL/AO/DISP) to `MaterialCard`. Verification PASSED 10/10 must-haves. 4 plans, 2 waves, parallel execution. Forward: remaining v1.20 milestone phases.
