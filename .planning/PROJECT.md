...
## Next Milestone Goals — v1.15 Architectural Toolbar Expansion

After v1.14 shipped real GLTF furniture, the obvious next gap is **the room itself**. The toolbar currently has walls, doors, windows, and ceilings — that's it. v1.15 expands the architectural primitive set so Jessica can model her actual future home, not just a rectangle with furniture in it.

**Four features in v1.15:**

1. **Wall cutaway mode (Phase 59 / CUTAWAY-01)** — In 3D, the wall closest to the camera blocks the view of the interior. Add a way to ghost or hide that wall so the room can be viewed from any angle, not just from above. Standard CAD feature. Issue [#21](https://github.com/micahbank2/room-cad-renderer/issues/21).
2. **Stairs (Phase 60 / STAIRS-01)** — Most homes have stairs. New architectural primitive: configurable rise / run / width / orientation. Renders as connected step boxes in 3D and as a stair-symbol polygon in 2D. Issue [#19 partial](https://github.com/micahbank2/room-cad-renderer/issues/19).
3. **Openings / archways (Phase 61 / OPEN-01)** — Most homes have non-rectangular wall openings between rooms (archways, pass-throughs, niches). Extends the existing wall-opening codepath (doors / windows) with new shapes. Issue [#19 partial](https://github.com/micahbank2/room-cad-renderer/issues/19).
4. **Measurement + annotation (Phase 62 / MEASURE-01)** — Dimension lines between any two points, object labels, automatic per-room area calculation in square feet. Communication tool — useful for verifying layouts and sharing plans. Issue [#22](https://github.com/micahbank2/room-cad-renderer/issues/22).

**Deferred to v1.16+:** columns ([#19](https://github.com/micahbank2/room-cad-renderer/issues/19) partial — niche), levels / platforms ([#19](https://github.com/micahbank2/room-cad-renderer/issues/19) partial — sunken rooms are uncommon), window presets ([#20](https://github.com/micahbank2/room-cad-renderer/issues/20) — current generic window already works for placement).

**Sequencing rationale:** Cutaway ships first because it's 3D-only and doesn't depend on new primitives — fastest visible value. Stairs second because it sets the new-primitive pattern. Openings third because it extends both the wall-cut codepath (existing doors/windows) and intersects with stairs in stairwell scenarios. Measurement last as the annotation layer that overlays everything.
